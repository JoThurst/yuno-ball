...
| `player_z_scores` | player | overwrite by player; no season/cutoff/version | Retained legacy values cannot establish historical context | read-only legacy analytics | player heat snapshots carry season/window/cutoff/version | Phase 4 disables writes and restores the PK; use the shared player snapshot contract |

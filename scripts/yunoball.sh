...
SCRIPTS_DIR="$(dirname "$0")"  # Get the directory where this script is located
PROJECT_ROOT="$(cd "$SCRIPTS_DIR/.." && pwd)"  # Get project root directory
...
case "${ARGS[0]}" in
    ...
    cron)
        ...
        if [ -f /sys/hypervisor/uuid ] && [ "$(head -c 3 /sys/hypervisor/uuid)" == "ec2" ]; then
            print_error "Cron job setup is disabled in AWS to prevent proxy overuse."
            print_error "Please set up data ingestion cron jobs locally instead."
            exit 1
        fi

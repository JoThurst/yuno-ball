"""Bounded, resumable backfill for Phase 2 player analytical snapshots."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill versioned player snapshots for scheduled NBA slate dates"
    )
    ...
    parser.add_argument(
        "--calculation-version",
        default=PLAYER_SNAPSHOT_CALCULATION_VERSION,
    )
    parser.add_argument(
        "--resume-after",
        type=_parse_date,
...
def _run_dates(
    ...
    *,
    ...
    totals = {"streaks": 0, "windows": 0, "heat": 0, "consistency": 0}
    for slate_date in slate_dates:
        ...
        counts = publish_player_snapshots(context, dry_run=dry_run)
...
def main() -> int:
    ...
    with IngestionRunTracker(
        run_type="player_snapshot_backfill",
        source="derived",
        season=season,
        season_type=season_type,

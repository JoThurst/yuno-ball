...
class Player:
    def __init__(self, player_id, name, position, weight, born_date, age, exp, school, available_seasons):
    ...
    def get_all_players(cls):
        ...
        cur.execute("SELECT player_id, name, position, weight, born_date, age, exp, school, available_seasons FROM players;")
        rows = cur.fetchall()
    ...
    def get_player(cls, player_id):
        ...
        cur.execute('SELECT player_id, name, position FROM players WHERE player_id = %s;', (player_id,))
        row = cur.fetchone()
        if row:
            return cls(*row)
        return None
    #outdated
    @classmethod
    def update_player(cls, player_id, name, team, position, weight, born_date, age, exp, school):
        ...
        if row:
            updates = []
            params = []
            fields = ["name", "team", "position", "weight", "born_date", "age", "exp", "school"]
...
class Statistics:
    def __init__(self, stat_id, player_id, game_date, points, rebounds, assists, steals, blocks):
    ...
    @classmethod
    def add_stat(cls, player_id, game_date, points, rebounds, assists, steals, blocks):
        ...
        conn.commit()
        return cls(stat_id, player_id, game_date, points, rebounds, assists, steals, blocks)
...
class Team:
    ...
    def get_team(cls, team_id):
        ...
        cur.execute('SELECT team_id, name, abbreviation FROM teams WHERE team_id = %s;', (team_id,))
        row = cur.fetchone()
        if row:
            return cls(*row)
        return None
    ...
    def get_roster(self):
        ...
        return cur.fetchall()  # Returns a list of tuples (player_id, player_name, player_number)

...
class Team:
    """
The `Team` class provides methods to manage basketball teams and their rosters.

This class represents a basketball team and includes functionality for creating
and managing the `teams` and `roster` tables in the database. It allows adding
new teams, retrieving existing teams, and managing player rosters associated
with teams.
...
- roster (list): List of players in the team's roster, represented as tuples
...
- add_to_roster(player_id, player_name, player_number, position, how_acquired, season):
  Adds a player to the team's roster, avoiding duplication through `ON CONFLICT`.
- get_roster():
  Retrieves the roster for the current team from the database, returning a list
  of tuples (player_id, player_name, player_number).
"""

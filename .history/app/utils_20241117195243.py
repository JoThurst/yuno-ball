...
from nba_api.stats.endpoints import playercareerstats, leaguegamefinder, commonteamroster, playerindex, leaguedashplayerstats, commonplayerinfo
...
#     #active_players_moreinfo= playerindex.PlayerIndex(active_nullable=True).get_data_frames()[0]
#     for player in all_players:
#         player_id = player["id"]
#         time.sleep(5)
#         cplayerinfoObj = commonplayerinfo.CommonPlayerInfo(player_id=player_id, timeout=300).get_data_frames[0]
...
#         available_seasons = availableSeasons # Calculate Available Seasons from from_year and to_year to 2017-18 2018-19 ...
#         name = player["full_name"]
#         position = cPlayerInfo['POSITION']  # nba_api doesn't provide position in the static API
#         weight= cPlayerInfo['WEIGHT']
#         born_date=cPlayerInfo['BIRTHDATE']
#         age= age #Calculate Age from Birthdate: Output Example "1998-03-03T00:00:00"
#         exp=cPlayerInfo['SEASON_EXP']
#         school=cPlayerInfo['SCHOOL']
        

#         # Add player to ChromaDB
#         Player.add_player(player_id, name, position, weight,born_date, age, exp,school, available_seasons= available_seasons)
...
def fetch_and_store_players():
    ...
    valid_seasons = [f"{year}-{str(year + 1)[-2:]}" for year in range(2015, 2025)]
    ...
    for player in all_players:
        ...
        try:
            ...
            cplayerinfo_obj = commonplayerinfo.CommonPlayerInfo(player_id=player_id, timeout=300)
            available_seasons_df = cplayerinfo_obj.get_data_frames()[1]  # Second DataFrame is AvailableSeasons
            cplayerinfo_data = cplayerinfo_obj.get_data_frames()[0].iloc[0]  # First DataFrame is CommonPlayerInfo
            ...
            available_seasons = [
                season for season in valid_seasons if int(season[:4]) >= from_year and int(season[:4]) <= to_year
            ]
            ...
            if available_seasons:
                ...
                Player.add_player(
                    ...
                    exp=exp,
                    school=school,
                    available_seasons=",".join(available_seasons)  # Store as comma-separated string
                )
                print(f"Player {name} (ID: {player_id}) added with seasons: {available_seasons}.")
            else:
                print(f"Player {name} (ID: {player_id}) has no valid seasons in the range.")
        except Exception as e:
            print(f"Error processing player {player['full_name']} (ID: {player_id}): {e}")
...
#     stats_df = career.get_data_frames()[0]  # Get the main DataFrame from the response
...
#         Statistics.add_stat(player_id, game_date, points, rebounds, assists, steals, blocks)
#     print(f"Career stats for player {player_id} have been stored successfully.")
...
def fetch_and_store_player_stats(player_id):
    ...
    if Statistics.stats_exist_for_player(player_id):
        print(f"Stats for player {player_id} already exist. Skipping API call.")
        return
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:  # Only retry if attempts are left
                ...
                print(f"Failed to fetch stats for player {player_id} after {retries} attempts.")
...
def fetch_and_store_current_rosters():
    ...
    for team in teams_list:
        ...
        for player in team_roster:
            ...
            if Player.player_exists(player_id):
                ...
                # Add position and how_aquired into roster, and position, age, height, weight, draft into player

                ##position = player.get('POSITION', 'Unknown')
                team_name = player['TeamID']

                #Player.update_player(player_id,player_name,player_number,position,weight, born_date,age,exp,school)
    ...
    #update_player_teams()
    print("All current NBA teams and their rosters have been successfully stored.")
...
def update_player_teams():
    """Update the team field in the players table using data from the roster table."""
    cur.execute('''
        UPDATE players
        SET team = teams.name
        FROM roster

...
def fetch_and_store_player_stats(player_id):
    ...
    try:
        ...
        # Handle issues with missing or malformed data
        print(f"Data error for player {player_id}: {e}")
        break  # Stop retrying as this likely won't be resolved by retries

    except OSError as e:
        # Handle issues related to networking or API calls
        print(f"Network or system error for player {player_id}: {e}")
        ...
        # Handle unexpected data type or value issues
        print(f"Value error for player {player_id}: {e}")
        break  # Likely not retryable
...
def update_player_teams():
    ...
    ''')
    conn.commit()
    print("All currently rostered players have been listed for their correct "
          "team.")
...
# great work today, work tomorrow on enhancing the static past stats we
# collect, and then make more ways to interact with
# the stats API
